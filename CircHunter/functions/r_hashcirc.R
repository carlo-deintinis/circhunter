...
converter <- function(element) {
	...
	if (substr(first_line,0,1) == ">"){
		...
		command <- paste("python /circhunter/functions/fasta_to_fastq.py", element, ">", newname)
...
splitter <- function(data.folder, num_parts) {
	...
	splitcommand <- paste("fastq-splitter.pl -n", num_parts, target_files)
...
r_hashcirc <- function(data.folder, rnaseq, junctionseqs, hcargs) {
	# =======================================
	...
	merge_all <- paste(
		...
		"paste id count > Read_count_temp ;",
		"echo -e 'ID\tCounts' | cat - Read_count_temp > Read_count ;",
		"rm *.num *temp readcount* id count ;",
		"chmod 666 *"
	)
